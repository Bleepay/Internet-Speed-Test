document.querySelector('button').addEventListener('click', (e) => {
    ...
    document.querySelector('.loader-items') .classList.add('hide');
    ...
    downloadSrc.onload = function () {
        ...
        var timeDuration = (time_end - time_start) /1000,
            ...
            const animate = () => {
                if ( i < totalSpeed ) {
                    document.querySelector('.items').innerHTML = i.toFixed(2) + '<small>Mbps</small>';
                    setTimeout(animate, 20)
                    i+=1.02;
                } else {
                    document.querySelector('.items').innerHTML = totalSpeed + '<small>Mbps</small>';
                }
            }
            animate();
       document.querySelector('.items').innerHTML = totalSpeed + '<small>Mbps</small>';
       document.querySelector('.loader-items') .classList.remove('hide');
       document.querySelector('.loader-items') .classList.add('result');
